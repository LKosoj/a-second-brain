...
- Prefer existing flat note paths and hub notes over inventing deeper folder trees.
- Use simple headings, short lists, and explicit context phrases before reaching for fancy Markdown features.
- Keep one durable fact in one canonical note; links should improve retrieval, not decorate the page.
...
Our processing markers (for example `<!-- d-brain:entry-status: already_processed -->`) stay as HTML comments since they're parsed by scripts.
...
- Invent deep vault paths when a flat existing note already exists
