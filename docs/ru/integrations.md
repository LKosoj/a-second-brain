...
Стандартный режим использует локальную модель эмбеддингов и не требует
OpenAI-совместимых переменных ниже. При первой индексации может загружаться
модель. Для удалённого провайдера эмбеддингов задайте `OPENAI_API_KEY` и
`EMB_MODEL`; для нестандартного совместимого endpoint также нужен `BASE_URL`.
`RERANK_MODEL` необязателен. `MODEL` относится к планировщику recall, а не к
индексации QMD.
