...
Grok CLI получает запрос аргументом команды, поэтому очень длинные промпты
могут упереться в лимит длины аргументов; остальные backend'ы принимают
запрос через стандартный ввод.

Backend `claude-tmux` — это тот же Claude Code, но запущенный не в headless-режиме
(`claude -p`), а в обычном интерактивном интерфейсе внутри отдельной сессии
tmux (терминальный мультиплексор: запускает программу в фоновом «экране», с
которым можно работать программно). Запрос вставляется в поле ввода через
буфер tmux, а ответ читается из журнала сессии, который Claude Code пишет в
`~/.claude/projects/<проект>/<session-id>.jsonl`. Режим нужен как запасной
