...
Каждая перезапись файла или правка frontmatter (служебного блока в начале
заметки), которую рантайм делает через проверяющие функции записи, попадает в
журнал операций `vault/.session/ops.jsonl` вместе со снимком прежнего
содержимого. `recover` откатывает все записанные изменения одного прогона:

```bash
a-second-brain recover daily-process-20260903-210000 --vault vault
```

`run_id` печатается в конце ночного дайджеста. Перемещения файлов в журнал не
попадают и не откатываются. Не запускайте `recover`, пока работает бот или
ночная обработка: они пишут в тот же журнал.

## `a-second-brain ops-prune`

Удаляет записи журнала и снимки старше N дней (по умолчанию 30):

```bash
a-second-brain ops-prune --vault vault --days 30
```

Ночной цикл и так подрезает журнал до окна в 30 дней, команда нужна только
для ручной чистки. Правило «не одновременно с ботом и ночной обработкой»
действует и здесь.
